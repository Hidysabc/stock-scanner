...
import sys
import json
from alphavantage_functions import get_historical_prices, get_price
...
def two_sigma(symbol,start_date, span):
    ...
    close = get_historical_prices(symbol,str(end_date_parse), start_date)
    '''
    ...
    for i in symbol_dict:
        if i == "HOLD":
            ...
            for j in symbol_dict[i]:
                ...
                symbol_current_price = np.float(get_price(symbol))
